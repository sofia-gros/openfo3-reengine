...
AIは開発を進める際、以下のルールを厳格に遵守してください。

1. **タスクの進捗更新**: 各タスクを完了したら、必ず `Assets/task.md` を更新して状況を詳細に記載すること。
2. **自動コミット**: 更新後、速やかに Git コミットを実行すること。（コミットメッセージは日本語ルールを遵守）。
3. **継続的な自動実行**: コミット完了後、停止せず次のタスクを自動的に開始すること。
4. **自律的行動**: **タスクは絶対に途中で止めないでください。** ユーザーへの確認や質問が必要なクリティカルな分岐点を除き、自律的にコーディングとテストを繰り返して開発を進めること。

---

## 🚀 直近のAI開発実行プラン (この順に自動進行してください)
...
- [x] **プラン3: 建築物の回転・向き不整合バグの解析と修正 (#12) ※最難関・ディープデバッグ**
  - メガトンなどの建築物ノードの向きが合わない原因を徹底解明。FO3空間のオイラー角を相似変換（$M_{godot} = R \cdot M_{fo3} \cdot R^{-1}$）でGodot空間に持ち込む修正を適用完了。これにより回転不整合バグが理論上完全に解決。
- [x] **プラン4: 地面バグの修正 (#19)**
  - TerrainBuilder.cs のメッシュ・LOD・コリジョンのポリゴン巻き順を時計回り(CW)から反時計回り(CCW)へ修正し、Godotの裏面カリングで地面が消える不具合を解消。また高さ計算における_heightExaggeration倍率のバグを取り除き、FO3基準の絶対高さに適合完了。
- [x] **プラン5: 開発用データの精緻なフィルタリング非表示処理 (#20)**
  - 必要な動的オブジェクトを誤消去せず、Bethesdaの開発用マーカーオブジェクト（sound, marker, editor, wind）のみを非表示にするよう `Megaton.cs` にフィルタを追加完了。さらに、NPCのデバッグ用名前ラベルも非表示化し、没入感を向上。また、テクスチャロード失敗時のフォールバックをマゼンタから透明テクスチャ（Alpha=0）へ変更し、デカールの白い枠が残る不具合を解消。
- [x] **プラン5.5: プレイヤーのスポーン位置・物理挙動・カメラの改善**
  - 初期ロードセルを `LoadCellAsync("Megaton")` からエクステリア用の `LoadWorldAsync("Megaton")` へ正しく修正し、スポーン位置をメガトン入り口（166, 205, 275）へ変更。落下時のセーフティテレポートの戻り先を動的な `SpawnPosition` へ改善。
  - キャラクターの `FloorMaxAngle` (65度)、`FloorSnapLength`、`SafeMargin` を調整し、精緻なTrimesh地形での引っ掛かりを軽減。
  - FlyCamera使用中（F2キー）に、TPSカメラがマウス入力を処理してしまいプレイヤーの向きが変わる不具合を修正。
- [ ] **プラン6: 大規模ソースコードの機能別リファクタリング**
  - Megaton.cs などの巨大ファイルを責務ごとに分割。
...
- [-] 全 100+ レコードタイプの完全パース対応
  ...
  - [x] NPC_, CREA, RACE, LVLN, LVLC — キャラクター/レベルドリスト
  - [x] QUST, DIAL, INFO, SCEN — クエスト/会話
  - [ ] ACHR, ACRE, REFR — 配置（NPC/クリーチャー）
  - [x] MGEF, SPEL, EFSH — 魔法/効果
  - [x] PERK, AVIF, GMST — ゲーム設定
  - [ ] CELL, WRLD, LAND, NAVM — ワールドジオメトリ（一部実装済み）
  - [x] SOUN, MUSC, WTHR — オーディオ/天候
  - [-] CSTY, GLOB, PMIS — AI/グローバル（CSTY, PMISは対応、GLOB未対応）
...
- [x] **全サウンド再生システム**: Wwise/FMOD 相当の動的オーディオ (AudioExpansion/AudioManagerにて実装)
- [-] BSA からのサウンド展開（XWM/MP3/WAV） (WAV, XWMデコード対応、OGG/MP3はRedot制約によりバイトロード未対応)
- [x] **3D 空間オーディオ**: Godot のオーディオエンジン + HRTF 対応。OpenMW より高精度な定位
- [ ] 環境音のゾーン配置（AMB レコード）
- [-] **ディレイ/リバーブ/オクルージョン**: 屋内と屋外の音響の差。閉じたドア越しの減衰 (ReverbZone, 簡易遮蔽実装)
- [-] **ラジオシステム**: Galaxy News Radio / Enclave Radio の選局 UI + ノイズ混信 (バックエンドロジック実装済み)
...
- [x] KF ファイル読み込み（NiSequence/NiControllerManager） (KFAnimationLoaderにてパース・生成)
- [-] アニメーションステートマシン（Idle/Walk/Run/Attack/Hit/Death） (AnimationManager/NpcAgentにて基礎連携)
- [ ] ブレンドツリー（上下半身分離 — 歩きながら射撃）
- [ ] 逆運動学 (IK) — 足の設置、ドアノブ操作
- [ ] ルートモーション（移動アニメーション）
- [⭐] **AI モーション生成**: 機械学習ベース of 自然なモーション補間
...
- [-] DIAL/INFO レコードからの会話ツリー (EsmParserパースおよびDialogueSystemでのロード)
- [-] スキルチェック（Speech/Barber） (DialogueSystemにて判定ロジック実装)
- [ ] 態度/好意度システム
- [ ] 音声付き会話（LIP ファイル同期）
...
- [x] **バットタイム**: 時間停止＋ターゲット選択 UI (VatsSystem)
- [x] **部位狙い**: 頭・腕・脚・胴体の個別ヒット判定 (VatsSystem)
- [x] **命中率表示**: 距離・武器スキル・遮蔽に基づく動的確率 (VatsSystem)
- [🔥] **肉弾カメラ**: 近接攻撃時のドラマチックカメラワーク
- [ ] クリティカルヒット演出
- [-] AP 消費/回復 (VatsSystemおよびPlayerControllerにて基礎実装)

### 7.2 武器/防具

- [-] 全武器種対応（拳銃・小銃・ショットガン・エネルギーの完全再現） (WeaponSystemでの武器種定義)
- [-] ダメージ計算（DMG/DATA → ダメージタイプ・DT・DR 解決） (WeaponSystem)
- [-] 武器コンディション（劣化/修理システム） (WeaponSystem)
- [-] 弾薬タイプ（通常・徹甲・散弾・エネルギーセル） (WeaponSystem)
- [-] 照準/ADS（アイアンサイト・スコープ） (ThirdPersonCamera & WeaponSystem)
- [ ] リロードアニメーション
- [ ] 武器改造（FO3 にはないが、NV/FO4 スタイルの改造を追加）

### 7.3 S.P.E.C.I.A.L / スキル/ パーク

- [x] 7 属性完全実装 (Strength, Perception, Endurance, Charisma, Intelligence, Agility, Luck) (CharacterStats)
- [x] 全 13 スキル（Small Guns, Big Guns, Energy Weapons, Explosives, Lockpick, Medicine, Melee, Repair, Science, Sneak, Speech, Barter, Unarmed） (CharacterStats)
- [ ] レベルアップ UI（Perk 選択画面）
- [🔥] **全 Perk 実装**（100+ Perk）
...
- [-] アイテム収集/インベントリ (PipBoyでのインベントリ/収集対応)
- [-] 武器修理（同一武器を消費） (CraftingSystemにて実装)
- [-] 薬品調合（化学実験台） (CraftingSystemにて実装)
- [ ] 弾薬製造（NV スタイル — オプション追加）
- [🔥] **ワークショップ**: FO4 スタイルの設置物建築（MOD では FO3 に逆移植実績あり）
- [-] スクラップ/解体 (CraftingSystemにて実装)
...
- [-] 体力/AP/放射線量インジケータ (HudOverlayにて一部実装)
- [ ] コンパス（目的地・施設アイコン）
- [ ] 敵対状態インジケータ（警戒/戦闘/搜索）
- [-] 武器/弾薬表示 (HudOverlayにて一部実装)
- [ ] サブタイトル/会話テキスト
- [ ] ターゲット情報（名前・HP・状態）

### 8.2 Pip-Boy 3000

- [-] **完全再現 Pip-Boy UI**: 緑色 CRT モニターのスタイルを完全再現 (UIパネル・タブバーの実装)
- [-] データタブ（ステータス/Perk/効果） (ステータス・SPECIAL・スキル表示実装)
- [-] インベントリ（武器/防具/アイテム/弾薬） (UI枠組み・カテゴリリスト実装)
- [-] マップ（Local/World マップ — 発見済みロケーション） (UIレイアウト実装)
- [-] ラジオ（選局/ON-OFF） (UIおよび局リスト実装)
- [-] クエストログ (クエストリストUI実装)
- [ ] 統計情報
...
- [-] QUST レコードからのクエストフル対応（全メイン+サイド+無線+DLC） (EsmParserでのパース、QuestManagerにてロード・管理)
- [x] クエストステージ/目標追跡 (QuestManager)
- [x] 条件分岐（複数解決方法） (QuestManager)
- [ ] クエストマーカー（コンパス/3D 空間）
...
- [x] NAVM レコード解析
- [x] NavigationMesh 生成
- [x] NavigationAgent3D 経路探索 (NpcAgent & NavMeshBuilder)
...
- [x] #10 メガトンの地面がない問題
- [x] #11 ライトの向きが正しくない問題
- [-] #12 街中の建築物の向きが合わない問題（※1か月未解決。慎重な検討が必要。解析にあたっては上位モデル（Gemini 1.5 Pro等）への切り替え推奨）
- [x] #13 Redot 26.1 の SurfaceTool に SetBoneIndices/SetBoneWeights がない問題（ArrayMesh直接アサインによる回避処理を適用済み）
- [ ] #14 大規模ワールドのロード時間が長い
- [ ] #15 一部 NIF メッシュのマテリアルが欠落（全 18 タイプ中の未対応 3 タイプ）
- [ ] #16 シャドウアクネ（近距離のシャドウノイズ）
- [ ] #17 水のレンダリング（NiWater/NiFluid 未対応）
- [ ] #18 ワールド切り替え時のメモリリーク
- [ ] #19 地面関連の問題（一部が表示されない、高さの不一致、テクスチャ不正）
- [ ] #20 開発用データの非表示処理（Bethesdaの開発用データが混入。必要なオブジェクトを削除しないフィルタリングロジックの実装）
- [x] #21 プレイヤー操作キャラクターの実装（ダミーから実際の操作キャラへの置き換え）およびオブジェクト衝突判定の追加

### リファクタリング

- [ ] 大規模ファイルの機能別・種類別への分割（人間が保守・開発しやすいサイズにする）
...
**レンダリング**: 45% · **地形**: 75% · **物理**: 75% · **サウンド**: 40% · **AI**: 20% · **UI**: 55% · **ゲームプレイ**: 40% · **MOD**: 30% · **ネットワーク**: 0%
...
### 2026-07-01: プレイヤーのコリジョンと奈落すり抜けバグ修正 (#21)

1. **プレイヤーコリジョン明示化**: `PlayerController.cs` で `CollisionLayer`/`CollisionMask` を 1 に明示し、当たり判定が正しく行われるようにした。
2. **落下防止セーフティガード**: 地形やオブジェクトのロードがフレーム遅延した場合に備え、Y < -50f に落下した際に Vault 101 開始位置へ安全にテレポートさせるロジックを `_PhysicsProcess` に追加。
3. **ロード完了とキュー処理の同期バグ修正**: `_inGame` が `false`（ロード待ち中など）の際、`_Process` 内のアセット生成キュー処理が一切走らないため、プレイヤー生成時点で衝突オブジェクトがまだシーン内に実体化しておらず、自由落下してすり抜けていたバグを修正。`_Process` で `_inGame` の成否に関わらず常にアセット生成キューをデキューして追加するよう修正した。
4. **ロード完了の await 同期**: `OnIntroComplete` や `LoadLastSaveGame` 内で非同期の `LoadWorldAsync` のタスク戻り値が破棄されていたものを、`await` してワールド全体のロードが終わってからプレイヤーを生成・スポーンするように変更。

---

### 2026-07-01: ボーンウェイト制約の代替実装とアニメーション構造の検証完了 (#13, #5.2)

1. **代替実装の確認**: Redot の SurfaceTool に SetBoneIndices / SetBoneWeights が無い問題について、すでに NIFMeshBuilder.cs:BuildSkinned にて ArrayMesh の頂点配列に Bones (int[]) と Weights (float[]) を直接流し込む代替実装が正しく記述され、ビルドも正常に通ることを検証した。
2. **ノードパス・プレフィックスの一致検証**: KFAnimationLoader.cs が出力するボーンのパスプレフィックス "NPCVisual/Skeleton:" が、NpcAgent.cs がアタッチするノード階層構成（NPCVisual の子としての Skeleton）と完全に一致しており、ランタイムで正常にアニメーションがボーンにマッピングされることを検証・確認した。
