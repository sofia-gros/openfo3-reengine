...
- [x] **プラン1: プレイヤーのコリジョンと実操作キャラクターの置き換え (#21)**
  - PlayerController.cs に CollisionLayer/Mask (1) を明示、落下防止ガード実装。非表示ロード中にキューがデキューされないバグ（_inGameがfalseのため）を修正し、ロードが完全に終わってコライダーがシーンに実体化してからプレイヤーを生成する同期処理に改修。
...
- [ ] **プラン3: 建築物の回転・向き不整合バグの解析と修正 (#12) ※最難関・ディープデバッグ**
  - メガトンなどの建築物ノードの向きが合わない原因を徹底解明。NIFReader/EsmParserでのオイラー角/回転行列変換、Pivotの原点ずれを修正。
- [ ] **プラン4: 地面バグの修正 (#19)**
  - 一部の地面が表示されない問題、高さの違い、テクスチャの不整合を修正。
- [ ] **プラン5: 開発用データの精緻なフィルタリング非表示処理 (#20)**
  - 必要な動的オブジェクトを誤消去せず、Bethesdaの開発用マーカーオブジェクトのみを非表示にする。
